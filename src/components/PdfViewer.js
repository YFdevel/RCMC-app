...
useEffect(() => {
    const checkFileExists = async () => {
        ...
        setError(null);
        try {
            const checkUrl = fileUrl.startsWith('/') ? `${process.env.PUBLIC_URL}${fileUrl}` : fileUrl;
            const response = await fetch(checkUrl, { method: 'HEAD' });
            if (response.ok) {
                setFileExists(true);
                setError(null);
...
const getPdfUrl = useCallback(() => {
    if (!fileUrl) return '';
    return fileUrl.startsWith('/') ? `${process.env.PUBLIC_URL}${fileUrl}` : fileUrl;
}, [fileUrl]);
...
const handleMultiTouchMove = useCallback((e) => {
    ...
    if (initialPinchDistanceRef.current > 0) {
        const newScale = Math.max(0.5, Math.min(3.0, initialScaleRef.current * (currentDistance / initialPinchDistanceRef.current)));
        setScale(newScale);
        if (Math.abs(currentDistance - lastTouchDistanceRef.current) > 5) {
            setShowZoomIndicator(true);
            lastTouchDistanceRef.current = currentDistance;
...
return (
    <div className="pdf-presentation-viewer" ref={containerRef}>
        {!isMobile && (
            <div className="pdf-controls">
                <div className="pdf-nav-controls">
                    <button onClick={goToPrevPage} disabled={pageNumber <= 1 || isLoading || error} className="pdf-control-btn" title="Предыдущая страница (←)">
                        <i className="fas fa-chevron-left"></i>
                    </button>
                    <div className="page-info">
                        <input
                            ...
                            onChange={(e) => {
                                const page = parseInt(e.target.value);
                                if (!isNaN(page) && page >= 1 && page <= (numPages || 1)) setPageNumber(page);
                            }}
                            min="1"
                            max={numPages || 1}
                            className="page-input"
                    ...
                    <button onClick={goToNextPage} disabled={pageNumber >= (numPages || 1) || isLoading || error} className="pdf-control-btn" title="Следующая страница (→)">
                        <i className="fas fa-chevron-right"></i>
                    </button>
                </div>
                <div className="pdf-zoom-controls">
                    <button onClick={zoomOut} disabled={scale <= 0.5 || isLoading || error} className="pdf-control-btn" title="Уменьшить масштаб (-)">
                        <i className="fas fa-search-minus"></i>
                    </button>
                    <span className="zoom-level">{Math.round(scale * 100)}%</span>
                    <button onClick={zoomIn} disabled={scale >= 3.0 || isLoading || error} className="pdf-control-btn" title="Увеличить масштаб (+)">
                        <i className="fas fa-search-plus"></i>
                    </button>
                    <button onClick={resetZoom} disabled={scale === 1.0 || isLoading || error} className="pdf-control-btn" title="Сбросить масштаб (0)">
                        <i className="fas fa-sync-alt"></i>
                    </button>
                </div>
            </div>
        ...
        <div className="pdf-viewport" ref={viewportRef}>
            ...
            {!error && fileExists && (
                <div className="pdf-document-container">
                    <Document
                        ...
                        onLoadSuccess={onDocumentLoadSuccess}
                        onLoadError={onDocumentLoadError}
                        loading={<div className="pdf-loading"><i className="fas fa-spinner fa-spin"></i><p>Загрузка PDF...</p></div>}
                    >
                        <Page
                            pageNumber={pageNumber}
                            scale={scale}
                            ...
                            renderTextLayer={!isMobile}
                            renderAnnotationLayer={!isMobile}
                            loading={<div className="pdf-loading"><i className="fas fa-spinner fa-spin"></i><p>Загрузка страницы...</p></div>}
                        />
                    </Document>
                </div>
            )}
            {isMobile && (
                <div className={`pdf-page-indicator ${showPageIndicator ? 'show' : ''}`}>
                    <div className="pdf-page-indicator-content">{pageNumber} / {numPages || '?'}</div>
                </div>
            )}
            {isMobile && (
                <div className={`pdf-zoom-indicator ${showZoomIndicator ? 'show' : ''}`}>{Math.round(scale * 100)}%</div>
            )}
            {isMobile && pageNumber === 1 && numPages > 1 && !showPageIndicator && scale === 1.0 && (
                <div className="pdf-swipe-hint">
                    <div className="pdf-swipe-hint-content"><i className="fas fa-arrow-left"></i><span>Свайп для перелистывания</span><i className="fas fa-arrow-right"></i></div>
                </div>
            )}
            {isMobile && scale === 1.0 && !showZoomIndicator && !showPageIndicator && (
                <div className="pdf-swipe-hint" style={{ bottom: '70px', animationDelay: '1s' }}>
                    <div className="pdf-swipe-hint-content"><i className="fas fa-search-plus"></i><span>Pinch для масштабирования</span><i className="fas fa-search-minus"></i></div>
                </div>
            )}
        </div>
    </div>
